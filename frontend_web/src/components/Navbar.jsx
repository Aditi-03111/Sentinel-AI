...
import { MessageSquare, HelpCircle, GraduationCap, Settings, GitBranch, Sun, Moon } from 'lucide-react';
import { motion } from 'framer-motion';
import { useTheme } from '../ThemeContext';

const GRADE_LABELS = Array.from({ length: 12 }, (_, i) => ({ value: i + 1, label: `Class ${i + 1}` }));

export default function Navbar({ activeTab, onTabChange, grade, onGradeChange, onGenerateQuiz, onGenerateMindMap, hasDocument, isSummarizing, isMindMapping }) {
    const { theme, toggle } = useTheme();
    const isDark = theme === 'dark';

    return (
        <div className="h-11 shrink-0 border-b flex items-center px-4 gap-3 z-20 transition-colors"
            ...
            {activeTab === 'quiz' && hasDocument && (
                <button onClick={onGenerateQuiz} disabled={isSummarizing}
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white text-[11px] font-semibold transition-colors">
                    {isSummarizing
            ...
            <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg border transition-colors"
                ...
                <select value={grade} onChange={(e) => onGradeChange(Number(e.target.value))}
                    className="border-none text-[11px] font-semibold focus:outline-none cursor-pointer pr-1 bg-transparent"
            ...
            <button onClick={toggle}
                className="w-7 h-7 rounded-lg flex items-center justify-center border transition-all hover:border-indigo-500/50"
                style={{ background: 'var(--bg-elevated)', borderColor: 'var(--border)' }}
            ...
            <div className="flex items-center gap-2 pl-2 border-l" style={{ borderColor: 'var(--border)' }}>
                <div className="w-6 h-6 rounded-full bg-gradient-to-br from-indigo-500 to-cyan-500 flex items-center justify-center text-[9px] font-bold text-white">
                    U
                </div>
